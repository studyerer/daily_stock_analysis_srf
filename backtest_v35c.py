"""
v35c：v35b + 收窄动量区间（MAX_MOM20=25%, MOM_PREFERRED=15%）
==================================================================================
对照实验 Step 3：在 v35b 基础上收窄动量区间——
  MAX_MOM20 从 40% 降到 25%，MOM_PREFERRED 从 25% 降到 15%。

  目的：过滤掉涨幅过大的追高票，只选"温和上涨"的标的。
  LLM 策略偏好稳健上涨，这里测试更保守的动量区间。

累计变更（vs v27b）：
  条件6：bias_ma5 < 8%           (来自 v35a)
  条件7：MA5 > MA10              (来自 v35b)
  参数：MAX_MOM20=25%, MOM_PREFERRED=15%  (v35c 新增)
...
def main():
    parser = argparse.ArgumentParser(description="v35c 回测")
    parser.add_argument("--start", type=str, default=DEFAULT_START)
    parser.add_argument("--end", type=str, default=DEFAULT_END)
    parser.add_argument("--refresh", action="store_true")
    args = parser.parse_args()
    print(f"\n{'='*75}")
    print(f"  v35c：v35b + 动量区间收窄（{MIN_MOM20}~{MAX_MOM20}%，优选<{MOM_PREFERRED}%）")
    print(f"{'='*75}")
    print(f"  区间：{args.start}~{args.end}")
    print(f"  ── 累计变更（vs v27b）──")
    print(f"    条件6：MA5乖离率 < {BIAS_MA5_MAX}%（来自v35a）")
    print(f"    条件7：MA5 > MA10（来自v35b）")
    print(f"    ★ 参数：MAX_MOM20={MAX_MOM20}%, MOM_PREFERRED={MOM_PREFERRED}%（v35c新增）")
    print(f"  ── 买入（其余同v27b）──")
