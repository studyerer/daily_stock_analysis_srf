"""
v35a：v27b + MA5乖离率过滤（< 8%）
==================================================================================
对照实验 Step 1：在 v27b 基础上仅新增一个条件——
  买入时 MA5 乖离率 < 8%，过滤掉"涨太远"的股票。

  目的：验证 LLM "严禁追高"约束对选股质量的影响。
  LLM 用 5% 作为硬上限，这里先用 8% 作为宽松版本。

与 v27b 的唯一差异：
  check_buy_v11 新增第6个条件：bias_ma5 < BIAS_MA5_MAX (8%)
...
def check_buy_v11(row):
    ...
    reason = f"回踩确认|放量{vol_ratio:.1f}x|动量{mom:.1f}%|ATR{atr_r*100:.1f}%{bias_tag}{wr_tag}"
...
def run_backtest(data):
    ...
    print(f"  买入：回踩确认 | 动量{MIN_MOM20}~{MAX_MOM20}%(优选<{MOM_PREFERRED}%) | ATR<{ATR_THRESHOLD}% | ★乖离<{BIAS_MA5_MAX}%")
...
def main():
    parser = argparse.ArgumentParser(description="v35a 回测")
    parser.add_argument("--start", type=str, default=DEFAULT_START)
    parser.add_argument("--end", type=str, default=DEFAULT_END)
    parser.add_argument("--refresh", action="store_true")
    args = parser.parse_args()
    print(f"\n{'='*75}")
    print(f"  v35a：v27b + MA5乖离率 < {BIAS_MA5_MAX}%")
    print(f"{'='*75}")
    print(f"  区间：{args.start}~{args.end}")
    print(f"  ── 与 v27b 的唯一差异 ──")
    print(f"    ★ 新增条件6：MA5乖离率 < {BIAS_MA5_MAX}%（LLM要求<5%，这里先用8%宽松版）")
    print(f"  ── 买入（其余同v27b）──")
