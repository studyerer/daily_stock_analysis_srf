"""
v35d：v35c + 近期急跌过滤（2日累计跌幅 > -5% 排除）
==================================================================================
对照实验 Step 4：在 v35c 基础上新增一个条件——
  排除近2日累计跌幅超过 -5% 的股票（可能是"跳水接飞刀"）。

  目的：避免在急跌中买入，等待企稳后再进场。
  这是 LLM 分析中经常扣分的场景——"近期大跌，风险较大"。

累计变更（vs v27b）：
  条件6：bias_ma5 < 8%                     (来自 v35a)
  条件7：MA5 > MA10                        (来自 v35b)
  参数：MAX_MOM20=25%, MOM_PREFERRED=15%   (来自 v35c)
  条件8：近2日累计跌幅 > -5%               (v35d 新增)
...
# ── Step2: 买入参数 ──
MIN_MOM20        = 5.0
MAX_MOM20        = 25.0       # v35c: 40→25
MOM_PREFERRED    = 15.0       # v35c: 25→15
VOLUME_RATIO     = 1.2
BREAKOUT_DAYS    = 20
PULLBACK_WINDOW  = 7
ATR_THRESHOLD    = 0.07
BIAS_MA5_MAX     = 8.0        # v35a
CRASH_2D_THRESHOLD = -5.0    # ★ v35d 新增：近2日累计跌幅阈值
...
def calc_indicators(df):
    ...
    # ★ v35d 新增：近2日累计涨跌幅
    df["change_2d"] = (c / c.shift(2) - 1) * 100
...
def check_buy_v11(row):
    ...
    if pd.notna(bias) and bias > BIAS_MA5_MAX:
        return False, "", 0
    # 7. 短期均线多头 MA5 > MA10 (v35b)
    ma5 = row.get("ma5", np.nan)
    ma10 = row.get("ma10", np.nan)
    if pd.notna(ma5) and pd.notna(ma10) and ma5 <= ma10:
        return False, "", 0
    # 8. ★ v35d 新增：近2日急跌过滤
    change_2d = row.get("change_2d", np.nan)
    if pd.notna(change_2d) and change_2d < CRASH_2D_THRESHOLD:
        return False, "", 0
    ...
    ma_tag = "|MA5>10"
    c2d_tag = f"|2d{change_2d:+.1f}%" if pd.notna(change_2d) else ""
    reason = f"回踩确认|放量{vol_ratio:.1f}x|动量{mom:.1f}%|ATR{atr_r*100:.1f}%{bias_tag}{ma_tag}{c2d_tag}{wr_tag}"
...
def run_backtest(data):
    ...
    print(f"  买入：回踩确认 | 动量{MIN_MOM20}~{MAX_MOM20}%(优选<{MOM_PREFERRED}%) | ATR<{ATR_THRESHOLD}% | 乖离<{BIAS_MA5_MAX}% | MA5>MA10 | ★2日跌>{CRASH_2D_THRESHOLD}%")
...
def main():
    parser = argparse.ArgumentParser(description="v35d 回测")
    parser.add_argument("--start", type=str, default=DEFAULT_START)
    parser.add_argument("--end", type=str, default=DEFAULT_END)
    parser.add_argument("--refresh", action="store_true")
    args = parser.parse_args()
    print(f"\n{'='*75}")
    print(f"  v35d：v35c + 近2日急跌过滤（>{CRASH_2D_THRESHOLD}%）")
    print(f"{'='*75}")
    print(f"  区间：{args.start}~{args.end}")
    print(f"  ── 累计变更（vs v27b）──")
    print(f"    条件6：MA5乖离率 < {BIAS_MA5_MAX}%（来自v35a）")
    print(f"    条件7：MA5 > MA10（来自v35b）")
    print(f"    参数：MAX_MOM20={MAX_MOM20}%, MOM_PREFERRED={MOM_PREFERRED}%（来自v35c）")
    print(f"    ★ 条件8：近2日跌幅 > {CRASH_2D_THRESHOLD}%（v35d新增）")
    print(f"  ── 买入（其余同v27b）──")
